...
> 表中所有时刻均为**东八区时间**，且该时点由配置 `report.hour` 决定（默认 5，可改成任意整点）。
...
| 字段 | 默认值 | 说明 |
|------|--------|------|
| `hour` | `5` | 每天生成**定时报告**的整点小时（0-23，**东八区**）。改成 `8` 即每天 8:00 生成前一天的报告；非法值（<0 或 >23）回退为 5 |
| `periodic_mail` | `true` | 每天定时生成的报告是否发送邮件；`false` 则只写 `reports/` 文件并打印到日志 |
| `startup_mail` | `false` | 启动时生成的日报/周报/月报/年报是否发送邮件；`true` 则启动即发 4 封邮件 |

启动时日志会打印当前生效值，便于核对：
```
报告配置: 定时报告时点=东八区 05 点 | 定时报告发邮件=true | 启动报告发邮件=false
```

> `hour` 每次判定都实时读取配置，改完配置文件后**下一小时即生效，无需重启**。
